...
import { retrieve, reset } from '../../actions/quantitativevalue/show';
import { del, loading, error } from '../../actions/quantitativevalue/delete';
import { list } from '../../actions/quantitativevalue/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.quantitativevalueCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.quantitativevalueUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="QuantitativeValue">
              <List title="title">
                {Show.renderRow('id', item['@id'])}
                {Show.renderRow('id', item['id'])}
                {Show.renderRow('value', item['value'])}
                {Show.renderRow('unitCode', item['unitCode'])}
...
const mapStateToProps = (state) => {
  return {
    error: state.quantitativevalue.show.error,
    loading: state.quantitativevalue.show.loading,
    retrieved: state.quantitativevalue.show.retrieved,
    deleteError: state.quantitativevalue.del.error,
    deleteLoading: state.quantitativevalue.del.loading,
    deleted: state.quantitativevalue.del.deleted,
  };
};
