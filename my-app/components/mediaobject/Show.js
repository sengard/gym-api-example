...
import { retrieve, reset } from '../../actions/mediaobject/show';
import { del, loading, error } from '../../actions/mediaobject/delete';
import { list } from '../../actions/mediaobject/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.mediaobjectCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.mediaobjectUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="MediaObject">
              <List title="title">
                {Show.renderRow('id', item['@id'])}
                {Show.renderRow('contentUrl', item['contentUrl'])}
              </List>
            </Card>
            }
...
const mapStateToProps = (state) => {
  return {
    error: state.mediaobject.show.error,
    loading: state.mediaobject.show.loading,
    retrieved: state.mediaobject.show.retrieved,
    deleteError: state.mediaobject.del.error,
    deleteLoading: state.mediaobject.del.loading,
